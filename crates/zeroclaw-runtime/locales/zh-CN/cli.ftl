...
turn-max-iterations-finalizer-prompt = 已达到工具调用次数上限。请仅根据目前已完成的工作给出你目前最好的回答，并概括已完成与仍未完成的部分。
turn-no-progress-finalizer-prompt = 工具循环已停止，因为重复检索没有带来更多有用信息。请仅使用本轮对话中已经收集到的证据回答用户。若证据不足，请明确说明哪些内容未能找到，不要编造事实。
